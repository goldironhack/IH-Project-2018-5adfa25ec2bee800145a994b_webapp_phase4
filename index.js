...
var namesBoro = ["Manhattan","Bronx","Brooklyn","Queens","Staten Island"];
    ...
    var dataH = $.get(NYhousing,function(){})
        .done(function(){            
            ...
            for(var i = 0; i< infoNYhousing.length;i++){
                infoNYhousing[i].sort(function(a,b){
                    return a[4]-b[4];
                });
            }            
            fillArrayHous();
...
function fillArrayHous(){
    var max=0,m=0,k=0;
    for (var i = 0; i < infoNYhousing.length; i++){
        // max = infoNYhousing[i][0][5];
        for (var j = 0; j < infoNYhousing[i].length; j++){
            max = Math.max(max,infoNYhousing[i][j][5]);  
            if(j>0){
                if(infoNYhousing[i][j][4]==infoNYhousing[i][j-1][4] &&infoNYhousing[i][j][4] !=304){                    
                    // console.log(max + " " + i + " " + j);console.log("---------")                  
                }else{
                    dataHous.push(max);
                    max = 0;
                }
        ...
        if(i!=4)dataHous.push(max);max=0;
...
function clearTable(){
    tableReference = document.getElementById("clasificationTable").rows;
    for(var i = 1; i < tableReference.length; i++){
        info = tableReference[i];
        info.innerHTML = "";
    }
    tableReference = document.getElementById("clasificationTable");
    for(var i = 1; i < tableReference.length; i++){
        info = tableReference;
        info.deleteRow(i);
    }
...
function fillTable(){
    tableReference = document.getElementById("clasificationTable");
    var c = 0;    console.log(table.length);
    for(var i = 0; i < distanceDistric.length;i++){
        for(var j=0;j<districH[i].length;j++){
